...
export function getNextHackingLevel(ns) {
    ns.disableLog('ALL');
    let hostnames = deepScan(ns, 'home');
    hostnames = hostnames.filter((hostname) => (ns.getServerRequiredHackingLevel(hostname) > ns.getHackingLevel()));
...
export async function scpAll(ns, filename = 'home') {
    ns.disableLog('ALL');
...
export function getPortFunctions(ns) {
    ...
    if (ns.fileExists('BruteSSH.exe'))
        portFunctions.push(ns.brutessh);
    if (ns.fileExists('FTPCrack.exe'))
        portFunctions.push(ns.ftpcrack);
    if (ns.fileExists('relaySMTP.exe'))
        portFunctions.push(ns.relaysmtp);
    if (ns.fileExists('HTTPWorm.exe'))
        portFunctions.push(ns.httpworm);
    if (ns.fileExists('SQLInject.exe'))
        portFunctions.push(ns.sqlinject);
...
export function findServer(ns, target, start = 'home', source = '') {
    ...
    if (hostnames.includes(target)) {
        if (start != 'home')
            return `connect ${start}; connect ${target}`;
    ...
    for (let i = 0; i < hostnames.length; ++i) {
        ...
        if (connection.length > 0) {
            if (start == 'home')
                ...
                return `connect ${start}; ${connection}`;
        }
    }
    return '';
}
...
export function formatRAM(ns, n) {
    ...
    if (n < 1e6)
        return ns.nFormat(n / 1e3, '0.00') + 'TB';
    if (n < 1e9)
        return ns.nFormat(n / 1e6, '0.00') + 'PB';
    if (n < 1e12)
        return ns.nFormat(n / 1e9, '0.00') + 'EB';
    return ns.nFormat(n, '0.00') + 'GB';
}
